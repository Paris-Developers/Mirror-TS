...
import { scheduleTimer, guildTimers } from '../resources/scheduleTimer';
		...
		try {
			...
			if (guildArray.length >= 10)
				return interaction.reply(
					'You have reached the maximum number of timers, please delete on before adding another'
				);
			...
			if (hour > 23 || hour < 0)
				return interaction.reply('Please enter a valid hour between 0 and 23');
			...
			if (interaction.options.getString('messageoptions') == 'weather') {
				try {
					...
					interaction.reply(
						'We has a problem constructing your message, try again'
					);
			...
			let response = (await interaction.reply({
				content:
					'Here is your timers content, react with ✅ if you want to continue to schedule this timer, and ❌ if not',
				embeds: [embed!],
				fetchReply: true,
			})) as Message;
