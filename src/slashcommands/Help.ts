...
export class Help implements SlashCommand {
	...
	async run(bot: Bot, interaction: CommandInteraction): Promise<void> {
		...
		let index = 0;
		let message = await interaction.reply({embeds: [embedArray[index]], fetchReply: true}) as Message; //fetch the reply and store it so we can react to it and use it in the collector
		await message.react('⏪');
		await message.react('⏩');
