...
exports.run = async (client, interaction) => { 
    ...
    for(let arg of options){
        ...
        client.log(`Sucessful addition of ${arg.value}`);
        ctr += 1;
    }
    ...
    collector.on('collect', (reaction, user) => {
        client.log(`Collected ${reaction.emoji.name} from ${user.tag}`);
        total += 1;
    ...
    collector.on('remove', (reaction , user) => {
        client.log(`Removed ${reaction.emoji.name} from ${user.tag}`);
        total = total - 1;
        emoteVal[reaction.emoji.name] -= 1; //saves number of votes for each emoji
        ctr = 0;
        for(let arg of options){ //rewrite the embed and send the edits
    ...
    collector.on('end',collected => {
        ...
        client.log(`Ending collection, Collected ${total} items. ${emoteVal}`);
    }) 
}
